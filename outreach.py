...
- Do NOT start with "Hi [name]," — start with the hook
...
LINKEDIN DM
---
[DM text — no salutation needed, start with the hook]
...
def extract_from_draft(draft_path: Path) -> dict:
    ...
    an existing tailored draft .txt file.
    """
    ...
    for i, chunk in enumerate(chunks):
        # Section 1 content — alignment analysis
        if "SECTION 1" in chunk and i + 1 < len(chunks):
            section1 = chunks[i + 1]
        ...
        if "SECTION 2" in chunk and i + 1 < len(chunks):
            section2 = chunks[i + 1]
            cl_match = re.search(
            ...
            if cl_match:
                result["cover_letter"] = cl_match.group(1).strip()

        # JD section
        if "JOB DESCRIPTION" in chunk and i + 1 < len(chunks):
...
def find_latest_draft(company: str, role: str) -> Path | None:
    ...
    # Try exact match first, then any draft in the folder
    matches = sorted(company_dir.glob(f"*{role_slug}*.txt"), reverse=True)
    if not matches:
        matches = sorted(company_dir.glob("*.txt"), reverse=True)
...
def parse_outreach_output(text: str) -> dict:
    ...
    for block in blocks:
        ...
        if "LINKEDIN DM" in first_line:
            current = "linkedin_dm"
            content = re.sub(r"^LINKEDIN DM\s*\n?", "", block, flags=re.IGNORECASE).strip()
            if content:
                result["linkedin_dm"] = content
        elif "EMAIL SUBJECT" in first_line:
            current = "email_subject"
            content = re.sub(r"^EMAIL SUBJECT\s*\n?", "", block, flags=re.IGNORECASE).strip()
            if content:
                result["email_subject"] = content
        elif "EMAIL BODY" in first_line:
            current = "email_body"
            content = re.sub(r"^EMAIL BODY\s*\n?", "", block, flags=re.IGNORECASE).strip()
            if content:
                result["email_body"] = content
        elif "WRITER" in first_line:
            current = "writers_notes"
            content = re.sub(r"^WRITER'?S? NOTES?\s*\n?", "", block, flags=re.IGNORECASE).strip()
            if content:
...
def save_outreach(
    ...
    """Save outreach messages to the company folder."""
    safe_company = re.sub(r'[/\\:*?"<>|]', '', company.strip())[:50]
    company_dir  = TAILORED_DIR / safe_company
    company_dir.mkdir(parents=True, exist_ok=True)

    role_slug = role.strip().replace(" ", "_").replace("/", "-")[:30]
    out_path  = company_dir / f"{safe_company}_{role_slug}_{today_str}_OUTREACH.txt"
